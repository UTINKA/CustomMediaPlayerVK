...
(function()
{
	...
	function Start()
	{
		new MutationObserver(function () 
		{
			...
			{
				CMPVKsetState(false);
				$('#settings_cmpvk_state').prop("checked", false);
			}
			else if(cmpvk[0] == 1)
			{
				CMPVKsetState(true);
				$('#settings_cmpvk_state').prop("checked", true);
			}
			if(cmpvk[1] == 0)
			{
				$('.top_audio_player').css('display','block');
				$('#settings_cmpvk_player').prop("checked", false);
			}
			else if(cmpvk[1] == 1)
			{
				$('.top_audio_player').css('display','none');
				$('#settings_cmpvk_player').prop("checked", true);
			}
			//
